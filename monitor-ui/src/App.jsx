...
export default function WorldMonitor() {
  ...
  useEffect(() => {
    ...
    // ── Threat Zones ──
    const threatCoords = [
      { lat: 32.4279, lon: 53.6880, radius: 0.08, name: "Iran" },
      { lat: 31.0461, lon: 34.8516, radius: 0.03, name: "Israel" }
    ];

    threatCoords.forEach(threat => {
      const pos = ll2v(threat.lat, threat.lon, 1.01);
      const threatGeo = new THREE.CircleGeometry(threat.radius, 32);
      ...
      threatMesh.position.copy(pos);
      threatMesh.lookAt(new THREE.Vector3(0, 0, 0));
      
      // Pulse animation
      threatMesh.userData = { isThreat: true, phase: Math.random() * Math.PI * 2 };
      grp.add(threatMesh);
      pulseRings.push(threatMesh);
    });
